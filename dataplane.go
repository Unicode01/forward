...
func (dp tcEBPFRuleDataplane) SupportsRule(rule Rule) (bool, string) {
	...
	if rule.Protocol != "tcp" && rule.Protocol != "udp" {
		return false, "kernel dataplane currently supports only transparent single-protocol TCP/UDP rules"
	}
	if !rule.Transparent {
		return false, "kernel dataplane currently supports only transparent single-protocol TCP/UDP rules"
	}
...
func applyKernelRuleSetConstraints(rules []Rule, plans map[int64]ruleDataplanePlan) map[int64]ruleDataplanePlan {
	...
	for _, ids := range grouped {
		...
		for _, id := range ids {
			...
			plan.FallbackReason = "kernel dataplane requires a unique backend TCP endpoint per active rule"
			plans[id] = plan
		}
	}
