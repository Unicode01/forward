(function () {
  ...
  app.translations = {
    'zh-CN': {
      ...
      'kernel.runtime.desc': '用于调试当前 TC/XDP 链路是否可用、是否已附加，以及是否存在临时回退。',
      ...
      'validation.sourceIPSpecific': '请使用明确且非回环的本机 IPv4 地址。',
    ...
    'en-US': {
      ...
      'kernel.runtime.desc': 'Debug the current TC/XDP dataplane state, attachment health, and transient fallbacks.',
      ...
      'validation.sourceIPSpecific': 'Use a specific non-loopback local IPv4 address.',
  ...
  app.populateSourceIPSelect = function populateSourceIPSelect(ifaceSel, inputEl, selected) {
    ...
    const appendOption = function appendOption(value, label) {
      ...
      if (value === '0.0.0.0' || /^127\./.test(value)) return;
      seen[value] = true;
      const opt = document.createElement('option');
      opt.value = value;
      opt.label = label;
