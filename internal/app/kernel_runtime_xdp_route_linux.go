...
func validateXDPDirectTarget(outLink netlink.Link, rule Rule) error {
	...
	if err := validateXDPNeighbor(outLink.Attrs().Index, nextHopIP, rule.OutInterface); err != nil {
		return err
	}
...
func validateXDPNeighbor(ifindex int, ip net.IP, ifName string) error {
	if ifindex <= 0 {
		return fmt.Errorf("xdp dataplane cannot resolve the outbound interface index")
	}
	ip4 := ip.To4()
	if ip4 == nil {
		return fmt.Errorf("xdp dataplane requires an IPv4 neighbor on %q", ifName)
	}

	neighbors, err := netlink.NeighList(ifindex, unix.AF_INET)
	if err != nil {
		return fmt.Errorf("xdp dataplane cannot inspect IPv4 neighbors on %q: %w", ifName, err)
	}
	...
	return fmt.Errorf("xdp dataplane requires a learned IPv4 neighbor entry for %s on %q", ip4.String(), ifName)
}
