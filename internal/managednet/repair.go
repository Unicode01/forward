...
func SummarizeRepairResult(result RepairResult) string {
	parts := make([]string, 0, 2)
	if summary := summarizeInterfaceSet(stringSliceToSet(result.Bridges)); summary != "" {
		parts = append(parts, "bridges="+summary)
	}
	if summary := summarizeInterfaceSet(stringSliceToSet(result.GuestLinks)); summary != "" {
		parts = append(parts, "guest_links="+summary)
	}
	return strings.Join(parts, " ")
}

func RepairResultInterfaceNames(result RepairResult) []string {
...
func ParsePVEBridgeBindings(vmid string, content string) []PVEBridgeBinding {
	nics := ParsePVEGuestNICs(vmid, content)
	if len(nics) == 0 {
		return nil
	}
	...
	for _, nic := range nics {
		...
		out = append(out, PVEBridgeBinding{
...
func parsePVEBridgeFromConfigValue(value string) string {
...
func BuildRepairInterfaceParentMap(infos []InterfaceInfo) map[string]string {
	if len(infos) == 0 {
		return nil
	}
...
func BuildRepairIssueMap(items []ManagedNetwork, ifaceParentByName map[string]string, bindings []PVEBridgeBinding) map[int64][]string {
	if len(items) == 0 {
		return nil
	}

	bindingsByBridge := make(map[string][]PVEBridgeBinding)
	...
	for _, item := range items {
		...
		for _, binding := range bindingsByBridge[bridge] {
			if detached, name := DetectDetachedPVEGuestLink(binding, bridge, ifaceParentByName); detached {
...
func DetectDetachedPVEGuestLink(binding PVEBridgeBinding, bridge string, ifaceParentByName map[string]string) (bool, string) {
	bridge = strings.TrimSpace(bridge)
	if bridge == "" || len(ifaceParentByName) == 0 {
		return false, ""
	}
	for _, candidate := range PVEGuestLinkCandidates(binding) {
...
func PVEGuestLinkCandidates(binding PVEBridgeBinding) []string {
