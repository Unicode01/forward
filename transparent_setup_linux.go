...
// ensureTransparentRouting sets up the ip rule, ip route, and iptables rules
// needed for IP_TRANSPARENT return traffic to be delivered to the proxy sockets.
func ensureTransparentRouting() {
	...
	// ip rule add fwmark 0x1/0x1 lookup 198
	if !ipRuleExists() {
	...
	// ip route replace local 0.0.0.0/0 dev lo table 198
	...
	// iptables -t mangle PREROUTING rules for tcp and udp
...
func ipRuleExists() bool {
	...
	// Match specifically "fwmark 0x1/0x1 lookup 198" to avoid false positives
